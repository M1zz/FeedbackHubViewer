...
private struct AllProjectsCard: View {
    ...
    var body: some View {
        NavigationLink(value: FeedbackStore.ProjectRoute(project: nil)) {
            CardFrame {
                CardTitle(systemImage: "square.grid.3x3",
                          tint: .purple,
                          name: "전체 프로젝트",
                          subtitle: "모든 앱을 한 번에")
                ...
                MetricRow(items: [
                    ...
                    .init(value: "\(store.allFeedback.count)", unit: "건", label: "피드백", tint: .primary),
                    .init(value: "\(store.unreadCount)", unit: "건", label: "안 읽음",
                          tint: store.unreadCount > 0 ? .red : .secondary),
                    // 안 읽음과 다른 숫자다: 읽었어도 반영/반영 안 함을 정하기
                    // 전까지는 여기 남는다.
                    .init(value: "\(store.pendingCount)", unit: "건", label: "확인 필요",
                          tint: store.pendingCount > 0 ? .orange : .secondary)
                ])

                DetailLine(text: detailText(stats: stats, traffic: traffic), traffic: traffic)
            }
        }
        .buttonStyle(.plain)
        .accessibilityLabel("전체 프로젝트, 최근 7일 사용 \(store.overallTraffic.events7)건, 피드백 \(store.allFeedback.count)건, 안 읽음 \(store.unreadCount)건, 확인 필요 \(store.pendingCount)건")
    }

    private func detailText(stats: FeedbackStore.Stats, traffic: FeedbackStore.Traffic) -> String {
        var parts: [String] = []
        if traffic.activeInstalls7 > 0 { parts.append("7일 활성 \(traffic.activeInstalls7)곳") }
        if let average = stats.averageRating { parts.append(String(format: "평균 별점 %.1f", average)) }
        if stats.last7Days > 0 { parts.append("7일 피드백 \(stats.last7Days)건") }
...
private struct ProjectCard: View {
    ...
    var body: some View {
        ...
        let crashes7 = store.crashSummary(for: summary.project).last7Days
        ...
        NavigationLink(value: FeedbackStore.ProjectRoute(project: summary.project)) {
            CardFrame {
                ...
                MetricRow(items: [
                    .init(value: "\(traffic.events7)", unit: "건", label: "7일 사용", tint: .accentColor),
                    .init(value: "\(traffic.installs)", unit: "곳", label: "설치", tint: .primary),
                    .init(value: "\(summary.count)", unit: "건", label: "피드백", tint: .primary),
                    .init(value: "\(summary.unreadCount)", unit: "건", label: "안 읽음",
                          tint: summary.unreadCount > 0 ? .red : .secondary),
                    .init(value: "\(summary.pendingCount)", unit: "건", label: "확인 필요",
                          tint: summary.pendingCount > 0 ? .orange : .secondary)
                ])
                ...
                DetailLine(text: detailText(traffic: traffic), traffic: traffic)
            }
        }
        .buttonStyle(.plain)
        .contextMenu {
        ...
        .accessibilityLabel("\(summary.displayName), 최근 7일 사용 \(traffic.events7)건, 피드백 \(summary.count)건, 안 읽음 \(summary.unreadCount)건, 확인 필요 \(summary.pendingCount)건")
    }

    /// 큰 숫자 네 개 밑에 붙는 보조 줄 — 있으면 도움이 되지만 크게 볼 필요는 없는 값들.
    private func detailText(traffic: FeedbackStore.Traffic) -> String {
        var parts: [String] = []
        if traffic.activeInstalls7 > 0 { parts.append("7일 활성 \(traffic.activeInstalls7)곳") }
        if let average = summary.averageRating { parts.append(String(format: "평균 별점 %.1f", average)) }
        if summary.last7Days > 0 { parts.append("7일 피드백 \(summary.last7Days)건") }
        if let latest = summary.latest { parts.append("마지막 \(AppFormat.relative(latest))") }
        return parts.joined(separator: " · ")
    }
}

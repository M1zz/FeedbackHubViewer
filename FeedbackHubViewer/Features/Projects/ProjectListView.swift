    ...
    var body: some View {
        CardFrame {
            ...
            MetricRow(items: [
                ...
                .init(value: "\(pendingCount)", unit: "건", label: "확인 필요",
                      tint: pendingCount > 0 ? .orange : .secondary)
            ])
...
private struct AllProjectsCard: View {
    ...
    var body: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel("전체 프로젝트, 최근 7일 사용 \(traffic.events7)건, 피드백 \(store.allFeedback.count)건, 안 읽음 \(store.unreadCount)건, 확인 필요 \(store.pendingCount)건")
    }
}

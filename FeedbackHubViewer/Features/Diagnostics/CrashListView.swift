...
struct CrashReportListView: View {
    ...
    private func headline(_ summary: FeedbackStore.CrashSummary) -> some View {
        HStack(spacing: 16) {
            Figure("최근 7일", "\(summary.last7Days)건",
                   note: "지난주 \(summary.previous7Days)건") {
                // No arrow before anything has been reported: "±0" against a
                // week that also had nothing is noise, not news.
                if summary.previous7Days > 0 || summary.last7Days > 0 {
                    DeltaLabel(value: summary.delta, polarity: .lowerIsBetter)
                }
            }

            Divider().frame(height: 34)

            Figure("전체", "\(summary.total)건",
                   note: summary.lastAt.map { "마지막 \(AppFormat.relative($0))" } ?? "—")
    ...
    private func kindPicker(_ summary: FeedbackStore.CrashSummary) -> some View {
        FilterChipRow(selection: $kind, choices:
            [(title: "전체 \(summary.total)", value: "")]
            + summary.byKind.map {
                (title: "\(CrashReport.label(for: $0.kind)) \($0.count)", value: $0.kind)
            })
    }
}
...
struct CrashRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack(spacing: 6) {
                if let projectLabel {
                    Tag(text: projectLabel, font: .caption2, isCompact: true)
                }

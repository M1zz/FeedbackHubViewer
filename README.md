...
- **정렬**: 프로젝트 목록·사이드바는 **트래픽이 많은 순**입니다 — 최근 7일 사용 건수(UsageEvent) →
  ...
  그 기준이 되는 숫자를 카드와 사이드바에 함께 적고("최근 7일 사용 N건 · 활성 M곳 · 설치 K곳"),
...
- **안 읽음 배지**: 상세를 연 피드백만 읽음으로 기록하고(기기별 `UserDefaults` 저장), 안 읽은 건수를
  ...
  지울 수 있습니다.
- **확인함(감추기)**: 목록의 각 줄 오른쪽에 있는 **확인** 동그라미를 누르면(또는 스와이프·우클릭,
  상세 화면의 큰 "확인함으로 표시하고 목록에서 감추기" 버튼) 그 피드백이 목록에서 빠집니다. 표시는 기기별 `UserDefaults`에 저장되어 앱을 다시 켜도 유지되고,
  **허브의 레코드는 건드리지 않습니다**. 상단 "확인함 N"(또는 "정렬 · 필터" 메뉴의 "확인한 피드백도
  보기")로 다시 꺼내 볼 수 있고, 같은 메뉴의 "확인 표시 모두 해제"로 되돌립니다. 상세를 여는 것만으로는
  감춰지지 않습니다 — 읽음과 확인함은 별개입니다.
...
└─ FeedbackHubViewer/
   ...
   ├─ FeedbackListView.swift       # 피드백 목록(확인함 감추기·정렬·필터)

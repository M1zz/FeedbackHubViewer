...
- **처리(반영/반영 안 함)**: 피드백마다 **반영함 / 반영 안 함**을 표시하고 메모("v1.4에 반영",
  "중복 피드백")를 남깁니다. 상세 화면 위쪽의 "처리" 칸, 목록 행 스와이프(왼쪽=반영함,
  오른쪽=반영 안 함), 행 우클릭 메뉴 어디서든 지정할 수 있고, macOS·iOS 모두 동일하게 동작합니다.
  - 목록은 기본적으로 **확인 필요만** 보여줍니다. 처리한 피드백은 목록에서 바로 빠지고,
    목록 맨 아래 **"처리한 피드백 N건 보기"**로 다시 볼 수 있습니다(상단 "상태" 메뉴에서
    확인 필요 / 처리 완료 / 반영함 / 반영 안 함 / 전체를 직접 고를 수도 있습니다).
  - 남은 건수는 macOS 목록 상단 바·목록 부제·프로젝트 카드(주황 배지)·개요 요약에 나옵니다.
    "상태" 메뉴의 **표시된 N건 모두 …**로 한 번에 처리할 수도 있습니다(확인 후 적용).

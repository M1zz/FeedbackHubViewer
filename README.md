...
- **키워드**: 프로젝트 화면의 **키워드** 세그먼트. App Store 검색에서 이 앱이 몇 위인지,
  ...
  **가장 가까운 자리**는 "어떤 검색어를 노려야 하나"에 답합니다. 순서를 정하는 건 순위가 아니라
  **위에 있는 앱 중 내 앱보다 리뷰가 많은 것의 수**입니다 — 18위인데 위 17개 중 7개가 나보다
  작으면 실제로는 10개 뒤이고, 21위인데 20개 중 13개가 작으면 7개 뒤라 더 가깝습니다.
  리뷰 수는 얼마나 자리 잡았는지를 재는 거친 대용이지만 **측정값**입니다(검색량처럼 지어낸
  값이 아닙니다). 컷오프는 두지 않았습니다 — "앞선 앱 5개 이하"를 넣어 봤더니 실제 데이터에서
  최선이 7개라 목록이 통째로 비었습니다. 아무도 재지 않은 숫자로 독자 대신 결정하는 게
  그런 컷오프가 하는 일입니다. 순서는 이쪽이 정하고, 어디서 끊을지는 보시는 분 몫입니다.
